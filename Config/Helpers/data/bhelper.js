...
const defaultTopics = [
    ["I. Обстановка:", "### Краткое общее описание происходящего - когда, где, кто и почему участвует в миссии. Общие мотивы сторон. Т.к. мы играем каждый раз в разных временных периодах, за разные стороны, то обстановка должна давать понять кто мы, что происходит и как мы до такого дошли."]
    ,["А. Враждебные силы:", "### Краткое описание известных сил противника (состав и количество), например:\n\n1 взвод легкой пехоты.\n\nТакже указывать известную или предпологаемую поддержку противника (бронетехника, минометы, гранатометы), описывать известные и ожидаемые места дислокации."]
    ,["Б. Дружественные силы:", "### Краткое описание сил игроков - 2 отделения Армии США: 1'1, 1'2. Детальное описание приветствуется. Например:\n\n1ый взвод 3ей роты 435ой парашютно-десантной дивизии тяжелых плуеметов:\n\n1`1 - отделение десантников (9 чел.),\n1`4 - отделение поддержки (7 чел., 2 пулемета, 1 ПТРК)"]
...
function generateTagsCheckboxes() {
    const tagsArray = Object.entries(defaultTags);
    const wrapper = document.querySelector("#tags > ul");
    for (let i = 0; i < tagsArray.length; ++i) {
        ...
        label.innerText = `${tagName} (${tagProps.hint})`;
...
function generateDefaultTopics() {
    const wrapper = document.querySelector("#briefing-form > ul");
    const parser = new DOMParser();
    let topicElement = null;
    for (let i = 0; i < defaultTopics.length; i++) {
        let content = defaultTopics[i];
        topicElement = parser.parseFromString(
            `<div id='topic${i}'><li><div class='dl-1'>Topic</div>
            <div class='dl-2'>
            <input class='topicInput' topicId='${i}' value='${content[0]}' placeholder='${content[0]}'></input>
            </div></li>
            <li><div class='dl-3'>
            <textarea class='topicData' cols='${textAreaSettings.cols}' 
            rows='${textAreaSettings.rows}'>${content[1]}</textarea>
            </div></li><hr /></div>`
            , "text/html"
        ).body.firstChild;
        wrapper.appendChild(topicElement);
    }
    document.querySelectorAll(".dl-3 > textarea").forEach(e => e.style.width = textAreaSettings.width);
}
...
function getCode() {
    ...
    const briefing = [];
    const topicTitles = document.getElementsByClassName("topicInput");
    const topicContents = document.getElementsByClassName("topicData");
...
function getCodeToDisplay() {
    var code = getCode();
    document.querySelector("#result-tab-data textarea").value = code;
    document.querySelector("#result-tab").style.top = "15%";
}
...
function getCodeToFile() {
    ...
    a.setAttribute('download', "_Briefing.sqf" );
    a.click();
}
...
function importBriefing(sqfText) {
    ...
    for (let i = 0; i < parsedTopics.length; i++) {
        populateTopic(i, parsedTopics[i][0], parsedTopics[i][1]);
    }
...
function populateTopic(id,title,text) {
    ...
    topic.getElementsByClassName("topicInput")[0].value = title;
    topic.getElementsByClassName("topicData")[0].value = text;
}

function populateTags(tags) {
    document.querySelectorAll("#tags input").forEach(e => e.checked = false);
    for (const tag of tags) {
        const tagId = normalizeTagname(tag)

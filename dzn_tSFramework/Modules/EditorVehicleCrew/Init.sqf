...
if (isServer) then {
	...
	tSF_fnc_EVC_assignCrew = {
		...
		_config params ["_roles","_side","_skill",["_kit",""],["_behavior",""],["_crewClass",""]];
		private ["_applyVehicleHold","_vehicleHoldAspect"];

		if (typename _roles != "ARRAY") then { _roles = [_roles]; };

		private _applyVehicleHold = !(_behavior isEqualTo "");
		...
		[_v, _side, _roles, if (_kit == "") then { nil } else { _kit }, _skill, _crewClass] call dzn_fnc_createVehicleCrew;

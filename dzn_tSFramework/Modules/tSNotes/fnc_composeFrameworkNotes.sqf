...
if (TSF_COMPONENT_ACTIVE(CCP)) then {
...
if (TSF_COMPONENT_ACTIVE(FARP)) then {
...
if (TSF_COMPONENT_ACTIVE(AirborneSupport)) then {
...
if (TSF_COMPONENT_ACTIVE(POM)) then {
...
if (TSF_COMPONENT_ACTIVE(MissionDefaults)) then {
    private _settings = ECOMPONENT_SETTINGS(MissionDefaults);
    if (_settings get Q(Calculator) get Q(enable)) then {
    ...
    if (_settings get Q(PhoneticAlphabet) get Q(enable)) then {
...
if (TSF_COMPONENT_ACTIVE(tSSettings)) then {
...
if (TSF_COMPONENT_ACTIVE(CrewOptions)) then {
...
[
    format ["Mission Framework (v%1)", TSF_VERSION_NUMBER],

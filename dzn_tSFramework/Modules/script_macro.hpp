...
#define PREP(FUNCNAME) FUNC(FUNCNAME) = compileScript ['COMPONENT_DATA_PATH(DOUBLES(fnc,FUNCNAME)).sqf']
...
// --- Module init
#define INCLUDE_MODULE(X) [] call compileScript ['MAINPREFIX\SUBPREFIX\X\data\PreInit.sqf']
#define RUN_MODULE(X) if (TRIPLES(PREFIX,module,X)) then { INCLUDE_MODULE(X); }

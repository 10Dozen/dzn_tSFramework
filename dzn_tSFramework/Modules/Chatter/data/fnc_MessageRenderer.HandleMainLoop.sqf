...
{
    private _queueID = _x;
    private _queue = [_queueID] call self_FUNC(getQueue);

    {
        private _qe = _x;
        _qe params ["_id", "_ctrls", "_state", "_ttl"];
        ...
        switch _state do {
            ...
            case STATE_PULL: {
                ...
                DEBUG_2("(Renderer.handleMainLoop) Addint to the trashcan: '%1' & %2", _queueID, _id);
                _trashcan append [[_queueID, _id]];
            };
            default {
...
//DEBUG_1("(Renderer.handleMainLoop) FINISHED LR queue handling at %1", time);

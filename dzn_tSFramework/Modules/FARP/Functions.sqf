...
tSF_fnc_FARP_spawnAssets = {
    ...
    _asset params ["_itemClassname", "_count"];
	if (_itemClassname in ["ACE_Wheel","ACE_Track"]) then {
		hint format ["%1 asset deployed", _assetName];
        #define ITEM_HEIGHT 0.38
        private _basicPos = ((tSF_FARP_Objects select { typeOf _x == "Land_CampingTable_F" }) select 0) modelToWorld [0,0.3,1 - ITEM_HEIGHT];
        for "_i" from 1 to _count do {
    		private _pos = [_basicPos # 0, _basicPos # 1, (_basicPos # 2) + ITEM_HEIGHT * _i];
    		private _asset = _itemClassname createVehicle _pos;
            _asset allowDamage false;
    		_asset setPosATL _pos;
            [_asset] spawn { sleep 2; (_this # 0) allowDamage true; };
        };
	} else {
		hint format ["%1 asset added to equipment box", _assetName];
		tSF_FARP_EquipmentBox addItemCargoGlobal [_itemClassname, _count];
	};
};
...
tSF_fnc_FARP_showMenu = {
	...
	// Assets
	private _assets = +tSF_FARP_Assets_BasicList;
	private _assetsNames = tSF_FARP_Assets_BasicList apply {
        _x params ["_item", "_count"];
        if (_count > 1) then {
            format ["%2x %1", (_item call dzn_fnc_getItemDisplayName), _count]
        } else {
            (_item call dzn_fnc_getItemDisplayName)
        }
    };
	...
	if (tSF_FARP_Assets_AllowPrimayMagazine && (primaryWeapon player) isNotEqualTo "") then {
        private _primaryMagazineClass = (getArray(configFile >> "CfgWeapons" >> primaryWeapon player >> "magazines")) select 0;
		_assets pushBack [_primaryMagazineClass, 5];
		_assetsNames pushBack format [
            "5x %1",
            _primaryMagazineClass call dzn_fnc_getItemDisplayName
        ];
	};
...
tSF_fnc_FARP_showServiceMenu = {
	...
	if (tSF_FARP_Repair_Allowed) then {
        ...
        _farpServiceMenu pushBack ([
            [_farpMenuLine,"SLIDER",[0, 100, _damageState]],
            [_farpMenuLine,"LABEL", ""]
        ] select tSF_FARP_Repair_Simple);
	...
	if (tSF_FARP_Refuel_Allowed) then {
		...
		_farpServiceMenu pushBack [_farpMenuLine,"CHECKBOX"];
        _farpServiceMenu pushBack ([
            [_farpMenuLine,"SLIDER",[0, 100, _fuel]],
            [_farpMenuLine,"LABEL", ""]
        ] select tSF_FARP_Refuel_Simple);
    ...
    if (tSF_FARP_Gear_Allowed) then {
        tSF_FARP_DialogLines pushBack "GEAR";
        _farpServiceMenu pushBack [_farpMenuLine,"LABEL","Refill equipment"];
        _farpServiceMenu pushBack [_farpMenuLine,"CHECKBOX"];
        _farpServiceMenu pushBack [_farpMenuLine,"LABEL",""];
        _farpMenuLine = _farpMenuLine + 1;
    };
...
tSF_fnc_FARP_ProcessService = {
	...
	*/
    ...
    private _needKitRenewal = false;

    private _dialogItemOffset = 0;
	{
		switch (true) do {
			case (_x == "REPAIR"): {
				_needRepair = _dialogResult # 0 # 0;
				_repairLevelRequested = 1;
                _dialogItemOffset = _dialogItemOffset + 1;

                if (!tSF_FARP_Repair_Simple) then {
                    _repairLevelRequested = (_dialogResult # 1 # 0)/100;
                    _dialogItemOffset = _dialogItemOffset + 1;
                };
			};
			case (_x == "REFUEL"): {
				_needRefuiling = _dialogResult # _dialogItemOffset # 0;
                _fuelLevelRequested = 1;
                _dialogItemOffset = _dialogItemOffset + 1;
                if (!tSF_FARP_Refuel_Simple) then {
                    _fuelLevelRequested = (_dialogResult # _dialogItemOffset # 0)/100;
                    _dialogItemOffset = _dialogItemOffset + 1;
                };
			};
			case (_x == "AMMO"): {
                _needRearmTotal = if (tSF_FARP_Rearm_Simple) then { _dialogResult # _dialogItemOffset # 0 } else { true };
                _ammoSectionStartId = _dialogItemOffset;
			};
            case (_x == "GEAR"): {
                _needKitRenewal = (_dialogResult select -1) # 0;
            };
		};
    ...
    if (_needKitRenewal) then {
        ...
        _sleepTime = _sleepTime + _gearRenewalCostTotal;
    };
...
tSF_fnc_FARP_getDefaultCargoLoadout = {
    params ["_veh"];
    private _cls = typeOF _veh;
    private _gear = [[], [], [], []];

    {
        (_gear # 0) pushBack [getText (_x >> "weapon"), getNumber (_x >> "count")]
    } forEach (configProperties [configfile >> "CfgVehicles" >> _cls >> "TransportWeapons"]);

    {
        (_gear # 1) pushBack [getText (_x >> "magazine"), getNumber (_x >> "count")]
    } forEach (configProperties [configfile >> "CfgVehicles" >> _cls >> "TransportMagazines"]);

    {
        (_gear # 2) pushBack [getText (_x >> "name"), getNumber (_x >> "count")]
    } forEach (configProperties [configfile >> "CfgVehicles" >> _cls >> "TransportItems"]);

    {
        (_gear # 3) pushBack [getText (_x >> "backpack"), getNumber (_x >> "count")]
    } forEach (configProperties [configfile >> "CfgVehicles" >> _cls >> "TransportBackpacks"]);

...
if (_ai isEqualTo []) exitWith {
    DEBUG_1("(expelCrew) No AI members",1);
};
...
private _expelExternalGroup = grpNull;
{
    ...
    if (_aiUnit isEqualTo _aiVehicle || { !isPlayer (effectiveCommander _aiVehicle) }) then {
        DEBUG_1("(expelCrew) Stopping unit out of vehicle / not under other player command",1);
        doStop _aiUnit;
    };
    ...
    if (isNull _grp) then {
        DEBUG_1("(expelCrew) Group is not defined in vehicle / not in vehicle - assign to extrenal",1);
        if (isNull _expelExternalGroup) then {
            DEBUG_1("(expelCrew) Create external group",1);
            _expelExternalGroup = createGroup (side player);
        };
        _grp = _expelExternalGroup;
    };
    
    DEBUG_1("(expelCrew) Expel to new group",1);

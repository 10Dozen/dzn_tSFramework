...
dzn_fnc_tSF_3DEN_AddSupportPoint = {
	private ["_objectVarName", "_objectName", "_displayName","_list","_attribute"];
	switch (toUpper _this) do {
		case "CCP": {
			_objectVarName = "dzn_tSF_3DEN_CCP";
			_objectName = "tSF_CCP";
			_displayName = "CCP";
			call compile preProcessFileLineNumbers "dzn_tSFramework\Modules\CCP\data\Compositions.sqf";
			_list = keys tSF_CCP_PrefabCompositions;
			_attribute = 'this setVariable ["tSF_CCP_Composition", "%1"]';
		};
		case "FARP": {
			...
			_attribute = 'tSF_FARP_Composition = "%1"';
		};
	};
	...
	collect3DENHistory {
		_attribute = format [_attribute, _compositionName];
		...
		call compile format ["
			...
			'tSF Tools - %3 was created (%5)' call dzn_fnc_tSF_3DEN_ShowNotif;
			"
			, _objectVarName
			, _objectName
			, _displayName
